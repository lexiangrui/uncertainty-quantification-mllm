...
def evaluate_response(response: str, expected_answer: str) -> dict[str, Any]:
    counts = {
        tag: len(re.findall(rf"</?{tag}>", response, flags=re.IGNORECASE))
        for tag in TAGS
    }
    tags_once = all(counts[tag] == 2 for tag in TAGS)
    opening_positions = [
        match.start() if (match := re.search(rf"<{tag}>", response, flags=re.IGNORECASE)) else -1
        for tag in TAGS
    ]
