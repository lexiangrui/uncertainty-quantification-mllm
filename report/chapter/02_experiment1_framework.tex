...
\toprule
\tableheadrow
\thead{Model} & \thead{Size} & \thead{Architectural characteristics} & \thead{Rationale for selection} \\
\midrule
...
\tablestriperow \textbf{LoRA rank / alpha / dropout} & 8 / 16 / 0.05 \\
\textbf{Target modules} & Language-model \texttt{q\_proj} and \texttt{v\_proj} \\
\tablestriperow \textbf{Learning rate / epochs} & $2\times10^{-4}$ / 1 \\
...
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\linewidth}{L{2.7cm}C{1.7cm}Y Y}
\toprule
\tableheadrow \thead{Dataset} & \thead{Question instances} & \thead{Data and task type} & \thead{Primary evaluation focus} \\
\midrule
\textbf{ViLP} & 900 & 300 questions $\times$ 3 question-image-answer (QIA) groups & Conflicts between language priors and factual or counter-prior visual evidence \\
\tablestriperow \textbf{HallusionBench} & 1,129 & 951 image-based questions and 178 non-image questions & Visual illusions, language priors, false premises, and image-text conflicts \\
\textbf{MM-Vet} & 218 & Single-image open-ended visual question answering across six core capabilities & Visual factual errors and compound capability failures in open-ended responses \\
\bottomrule
\end{tabularx}
\end{table}
