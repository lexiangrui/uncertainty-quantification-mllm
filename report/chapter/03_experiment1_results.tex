...
The three models produced 6,741 responses in total; 6,667 satisfied the XML requirements and entered the formal evaluation, yielding a validity rate of 98.9\%. Cell-level exclusions are reported in Appendix~\ref{subsec:appendix_exclusions}. This high validity rate indicates that format adaptation reliably supports the subsequent correctness, hallucination, and uncertainty analyses. LLaVA has the lowest overall accuracy (0.493) and the highest hallucination rate (0.679), whereas Qwen and InternVL achieve higher accuracy with substantially lower hallucination rates of 0.358 and 0.322, respectively; InternVL has the strongest overall performance.
...
As shown in \Cref{fig:label_joint}, correctness and hallucination are distinct evaluation targets. Among the 6,667 valid responses, 14.0\% are correct but hallucinated and 8.8\% are incorrect without hallucination. Replacing $H$ with the error label $E=1-C$ would therefore mismatch 1,522 samples (22.8\%). Moreover, 30.9\% of all hallucinated responses still have correct final answers, while 22.1\% of all incorrect responses contain no hallucination. Complete counts and conditional proportions are provided in Appendix~\ref{subsec:appendix_label_stats}. The absolute $\phi$ coefficients in the nine model--dataset cells range from 0.46 to 0.65, indicating that the labels are moderately to strongly associated but remain far from equivalent.
...
As shown in \Cref{fig:error_detection}, Semantic Entropy achieves the best AUROC in eight of the nine model--dataset cells, with a nine-cell mean of 0.724. UMPIRE is best only for LLaVA/HallusionBench, while PPL is not best in any cell. The mean ranks of SE, UMPIRE, and PPL are 1.22, 2.11, and 2.67, respectively. The model-wise macro-average SE AUROC is also consistently highest (0.672, 0.729, and 0.771 for LLaVA, Qwen, and InternVL), while all three methods generally perform better on MM-Vet than on ViLP and HallusionBench. Full cell-level AUROC values and confidence intervals are reported in Appendix~\ref{subsec:appendix_error_auroc}.
...
At $\alpha=0.25$, LUH shares range from 0.062 to 0.246, corresponding to 45--292 missed hallucinations. At $\alpha=0.50$, the shares increase to 0.268--0.515 and the counts to 193--680. Except for Qwen/PPL at the median threshold, the shares remain below their corresponding $\alpha$, indicating useful ranking ability but a substantial residual blind spot. Each row defines a method-specific LUH subset; subsets may overlap and should not be summed across methods.
...
\includegraphics[width=0.9\linewidth]{vilp_55_case2}
...
\begin{minipage}[t]{0.3\linewidth}
\vspace{0pt}\centering
\includegraphics[width=0.9\linewidth]{mmvet_v1_16}
\par\smallskip
{\footnotesize MM-Vet \texttt{v1\_16}}
\end{minipage}\hfill
\begin{minipage}[t]{0.65\linewidth}
\vspace{0pt}\footnotesize\raggedright\comicfont
\textbf{Response:}
...
The difficult subset contains positive and negative samples that receive similar scores from the three UQ methods but have opposite hallucination labels:
...
Candidate-pool sizes, score alignment, matching distances, and final subset composition are reported in Appendix~\ref{subsubsec:appendix_subset_details}.
