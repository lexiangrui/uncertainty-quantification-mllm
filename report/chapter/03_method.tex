...
\textbf{Evaluated models and local deployment.} Experiment~1 evaluates three open-source multimodal large language models, as summarized in \Cref{tab:evaluated_models}: LLaVA-1.5-7B, Qwen2.5-VL-7B-Instruct, and InternVL3.5-8B-HF. They represent a classical vision-language baseline, a recent native multimodal architecture, and a third open-source architecture, respectively. Because all three models must expose per-token logits during generation and the final-layer hidden states required by UMPIRE, I deploy them locally rather than access them through APIs. Model weights are stored locally in Hugging Face Transformers format and loaded in offline mode for generation. Inference uses Transformers with FlashAttention-2.
...
\addlinespace[3pt]
\textbf{InternVL3.5-8B-HF}\newline {\footnotesize\color{gray!75!black}\nolinkurl{OpenGVLab/InternVL3_5-8B-HF}} & $\sim$8B & Native Hugging Face multimodal checkpoint & Third model architecture, included to broaden coverage \\
\bottomrule
\end{tabularx}
\end{table}
...
All three models use the same training configuration, reported in \Cref{tab:lora_config}. LoRA updates only the attention projections \texttt{q\_proj} and \texttt{v\_proj} in the language-model component, while the vision encoder and multimodal alignment component remain frozen. The format fine-tuning therefore does not alter the models' visual perception or image-text alignment capabilities, which avoids confounding format adaptation with changes in hallucination rate or answer accuracy. If the vision component or all model parameters were fine-tuned directly, shifts in the response distribution caused by format adaptation would become entangled with changes in task capability, making it impossible to determine whether subsequent differences arose from the response protocol or from the model itself. Appendix~A.8 describes the comparison used to examine this freezing strategy, including variants that unfreeze the vision encoder or fine-tune all parameters. The validation losses of the three adapters are 0.765 for LLaVA, 0.693 for Qwen, and 0.633 for InternVL. Further details of data construction, training, and acceptance checks are provided in Appendix~A.7.

\begin{table}[H]
\centering
\caption{Format-oriented LoRA configuration. All three models use the same data split, target response format, and hyperparameters.}
\label{tab:lora_config}
\small
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{0.94\linewidth}{@{}L{3.7cm}Y@{}}
\toprule
...
\textbf{Example response.} Consider the response of InternVL3.5-8B-HF to a ViLP example, \texttt{vilp-0-case1}, whose correct answer is 4. The question illustrates the construction of ViLP: its initial prior statement, ``Modern drones typically have four propellers,'' agrees with the visual evidence. The model produces the following complete and well-formed single-line XML response. Each of the three tags occurs exactly once in the prescribed order, with no additional text outside the tags, allowing direct parsing by both the judge and the UQ pipeline.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.33\linewidth]{vilp_0_case1}
\caption{Example image for ViLP \texttt{vilp-0-case1}.}
...
The 900 ViLP instances are 900 evaluated QIA triplets formed from 300 question texts and three image-answer pairings per question, where QIA denotes Question--Image--Answer. The 1,129 HallusionBench instances combine the official image split (951) and non-image split (178); no image is supplied for non-image examples. The construction, question types, and evaluation focus of each dataset are detailed in Appendix~A.1.
...
Semantic Entropy and UMPIRE share exactly the same sampled responses so that sampling variation does not confound their comparison. During generation, I record the per-token log probabilities of the final answer and the final-layer hidden states, the latter only for UMPIRE. Parsing failures are not silently repaired; instead, they are explicitly reported as experimental outcomes, as described in Appendix~A.5.
...
The judge instructions and output format are provided in Appendix~A.6. Because the two labels apply to different parts of a response and are assigned independently, they permit the four combinations in \Cref{tab:correctness_hallucination_labels}. Subsequent UQ evaluation can therefore address error detection and hallucination detection as distinct targets.

\begin{table}[H]
\centering
\caption{Four combinations of the correctness label $C$ and hallucination label $H$.}
\label{tab:correctness_hallucination_labels}
\small
\setlength{\tabcolsep}{8pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{0.82\linewidth}{@{}C{1.2cm}C{1.2cm}Y@{}}
\toprule
$C$ & $H$ & Interpretation \\
\midrule
1 & 0 & The final answer is correct, and neither the visual observation nor the reasoning contains hallucination \\
1 & 1 & The final answer is correct, but the visual observation or reasoning contains hallucination \\
0 & 0 & The final answer is incorrect, but neither the visual observation nor the reasoning contains hallucination \\
0 & 1 & The final answer is incorrect, and the visual observation or reasoning contains hallucination \\
\bottomrule
\end{tabularx}
\end{table}
...
All metrics are computed independently within each model-dataset cell; raw scores are never compared across models or datasets. The metrics comprise label statistics, ranking performance, and blind-spot statistics, which respectively address the label distribution, the ability of a score to rank samples, and the amount of hallucination concealed in low-score regions.
