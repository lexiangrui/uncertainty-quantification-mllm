...
#### 1.6.1 实验一：现有 UQ 方法评估与共识 LUH 子集提取

实验一在统一的模型与数据集评测矩阵上生成结构化回答，完成正确性与幻觉的独立标注，计算 Perplexity、Semantic Entropy 与 UMPIRE 三种基线分数并评估其错误与幻觉检测能力；随后为每个模型提取 200 条低不确定性幻觉正类与 200 条低不确定性非幻觉负类，构成固定的 400 样本评测子集，作为实验二的评测基础。数据集、模型、标注协议、UQ 计算与子集提取流程将在第 2 章详细展开。

#### 1.6.2 实验二：改进 UQ 方法以识别低不确定性幻觉

实验二在实验一固定的低不确定性幻觉子集上设计新的 UQ 方法，使新分数能够识别三种基线漏检的低不确定性幻觉。新方法与基线使用完全相同的样本、幻觉标签与评测指标，验收须同时满足三个条件：子集上排序指标显著提升、完整测试集性能不退化、跨模型跨数据集稳定复现。具体的方法设计与验证过程将在后文详述。
