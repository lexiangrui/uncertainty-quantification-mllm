#!/usr/bin/env python3
"""Evaluation of ERA (Early Rationale Attribution) on the LUH subsets.

The method averages per-layer U_ERA over early decoder layers (default: 0 and 1).
Methods compared on identical samples and judge labels:
  PPL / SE / UMPIRE (baselines) and ERA (U_ERA @ layers 0-1).
"""
...
from src.improvement.era import DIRECT_LAYERS, FEATURES, layer_features
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate ERA vs baselines on LUH subsets")
    ...
    results = {}
    print(f"ERA layers: {args.layers}")
    ...
    for model in args.models:
        ...
        if not valid_ids:
            raise ValueError(f"{model}: no overlapping samples between subset, ERA, UQ, and judge")
        sids = valid_ids
