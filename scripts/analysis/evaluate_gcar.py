#!/usr/bin/env python3
"""Final evaluation of GCAR on LUH subsets / full test set.

GCAR — Generated-Context Attention Ratio:

    s_GCAR = A_prelim^(0..3) / (A_vis + A_prelim + A_text)^(0..3)
...
Reports AUROC (group-level cluster bootstrap CI), AUPRC, and PRR for
PPL / SE / UMPIRE / GCAR per model.
"""
...
_GCAR_LAYERS = 4  # first four decoder layers, frozen


def gcar_score(v: dict) -> float | None:
    """GCAR from the five-region components (first four layers).
    ...
    rows = [lb.get(str(i)) for i in range(_GCAR_LAYERS)]
    if not all(rows):
        return None
...
def load_all(model, subset_ids, group_ids):
    ...
    pool = subset_ids if subset_ids is not None else set(gcar) | set(uq) | set(judge)
    sids = sorted(s for s in pool if s in gcar and s in uq and s in judge)
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=PROJECT_ROOT / "results/analysis/luh/gcar_final_evaluation.json")
    ...
    for model in MODELS:
        ...
        for m in METHODS:
            ...
            metrics = {
                ...
                "auprc": auprc(sc, labels), "prr": prr(sc, labels),
            }
