...
from src.llm_judge.open_source import _backend_kind
...
def test_open_source_judge_selects_qwen_backend_by_model_name():
    assert _backend_kind("Qwen2.5-3B-Instruct") == "text"
    assert _backend_kind("/opt/lexiangrui/models/Qwen3-4B-Instruct-2507") == "text"
    assert _backend_kind("Qwen3.6-VL") == "vl"
    assert OpenSourceJudge.__name__ == "OpenSourceJudge"


def test_nli_judge_exposes_single_pair_interface():
    class FakeNLIJudge(NLIJudge):
        def check_pairs(self, pairs):
            assert pairs == [("premise", "hypothesis")]
            return [True]

    judge = object.__new__(FakeNLIJudge)
    assert judge.judge("premise", "hypothesis") is True


def test_strict_llm_verdict():
    assert parse_text_judge_verdict('{"verdict":"CORRECT"}')
    assert not parse_text_judge_verdict('{"verdict":"WRONG"}')
    prompt = json.loads(build_text_judge_prompt("Where?", ["left"], "on the left"))
...
def test_mmhal_response_rejects_malformed():
    for bad in [
        ...
        try:
            parse_multimodal_judge_response(bad)
        except ValueError:
            continue
...
def test_mmhal_messages_shape():
    messages = build_multimodal_judge_messages("mmvet", "<img>", "Q?", "ref", "pred")
